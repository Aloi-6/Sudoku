...
def diag(A, B):
    "produce diagonal labels of elements in A and elements in B. (have to be same size)"
...
def naked_twins(values):
    ...
    #this part has been fixed by the reviewer
...
def remove_val(grid, units, pos, val):
    """
    Removes all "val" values from the "grid" dictionary (sudoku grid), unless 
    except the box labeled as "pos"
    """
...
def eliminate(values):
    """
    If a digit has been used, remove it from possibilities from other boxes in the same unit
    """
    for row in row_units:
        for pos in row:
            val = values[pos]
...
def only_choice(values):
    """
    If a box has the only unique digit in a unit, we must pick it (only choice)
    """
    for unit in unitlist:
